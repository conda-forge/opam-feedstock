...
# CPPO BUILD SCRIPT
# ==============================================================================
# Build the cppo preprocessor for OCaml.
#
# CRITICAL: cppo is a BUILD-TIME tool that runs on the BUILD machine.
# In cross-compilation scenarios, cppo MUST be built as BUILD arch (x86_64),
# not TARGET arch (aarch64/ppc64le).
#
# Supported platforms:
# - Native Linux/macOS builds (all architectures)
# - Cross-compilation (cppo always built for BUILD arch)
# - Windows (MinGW-based builds with MSYS2)
#
# Based on dune output pattern (lines 201-380) from opam-feedstock recipe.
# ==============================================================================
...
# cppo source directory (multi-source recipe)
if [[ -d "${SRC_DIR}/Makefile" ]] && grep -q "cppo" "${SRC_DIR}/Makefile" 2>/dev/null; then
  CPPO_SRC="${SRC_DIR}"
elif [[ -d "${SRC_DIR}/cppo" ]]; then
  CPPO_SRC="${SRC_DIR}/cppo"
else
  echo "ERROR: Cannot find cppo source directory (expected Makefile with cppo target)"
  exit 1
fi
...
dune install --prefix="${CPPO_INSTALL_PREFIX}" --libdir="${CPPO_INSTALL_PREFIX}"/lib/ocaml
